...
import { QIcon, QBtn, QSlideTransition, QTooltip, QSpinner } from 'quasar';
import { shouldIgnoreKey } from 'quasar/src/utils/private/key-composition';
    ...
    setup(props, { slots, emit }) {
          ...
          if (isParent === true) {
            ...
            if (m.matchesFilter === true) {
              ...
              if (leafTicking) {
                ...
                m.tickable =
                  m.tickable === true &&
                  m.children.some((node) => node.tickable);
      ...
      function getSlotScope(node, meta, key) {
        ...
        injectProp(
          ...
          (val) => {
            val !== meta.expanded && setExpanded(key, val);
          }
        );
        ...
        injectProp(
          ...
          (val) => {
            val !== meta.ticked && setTicked([key], val);
          }
        );
      ...
      function getActionButton(event, nodeId, tooltip, icon) {
        return h(
          QBtn,
          {
            onClick: (e) => {
      ...
      function getNodeActions(node) {
        ...
        if (node.type === 'repo' || node.type === 'tree') {
          if (props.showManagementUI) {
            actions.push(
              getActionButton('action:upload', node.id, 'Upload', 'mdi-upload')
            );
          }
        }
        return actions;
      ...
      function getNode(node) {
        ...
        const isParent =
          children.length !== 0 || (m.lazy && m.lazy !== 'loaded');
      ...
      function onClick(node, meta, e, keyboard) {
        ...
        if (hasSelection.value && meta.selectable) {
          if (props.noSelectionUnset === false) {
            emit(
              'update:selected',
              meta.key !== props.selected ? meta.key : null
            );
